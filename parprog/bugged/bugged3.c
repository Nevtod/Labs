...
void print_results(float array[N], int tid, int section) 
{
    ...
    // #pragma omp barrier     Должны убрать этот барьер.
    //                         потому что иначе потоки, выполняющие секции,
    //                         зависнут на следующем барьере, который остальные
    //                         потоки пройдут, когда эти 2 будут здесь
